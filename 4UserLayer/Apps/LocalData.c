...
//            log_d(">>>>>>>>>>>>>___________________\r\n");
	...
	iTime2 = xTaskGetTickCount();	/* 记下结束时间 */
	log_d ( "ef_set_env_blob 成功，耗时: %dms\r\n",iTime2 - iTime1 );
	return 0;
...
//    char buff[SECTOR_SIZE] = {0};
	...
	if ( mode == CARD_MODE )
	{
		...
		curIndex = gCurCardHeaderIndex;
	}
	else if ( mode == USER_MODE )
	{
		address = USER_ID_HEAD_ADDR;
		curIndex = gCurUserHeaderIndex;		
	}
	else if ( mode == CARD_DEL_MODE )
	{
		address = CARD_DEL_HEAD_ADDR;
		curIndex = gDelCardHeaderIndex;		
	}
	else if ( mode == USER_DEL_HEAD_ADDR )
	{
		address = USER_DEL_HEAD_ADDR;
		curIndex = gDelUserHeaderIndex;
	}	
