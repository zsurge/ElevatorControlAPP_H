...
void Eth_Link_ITHandler ( struct netif *netif )
{
	...
	if (!(ETH_ReadPHYRegister(LAN8720_PHY_ADDRESS, PHY_BSR) & PHY_Linked_Status)) /*检测插拔中断*/
	{
        
		status = ETH_ReadPHYRegister ( LAN8720_PHY_ADDRESS, PHY_BSR );

        printf("lost ethernet link status = %d\r\n",status);
        
		if ( status & ( PHY_AutoNego_Complete | PHY_Linked_Status ) ) /*检测到网线连接*/
		{
			if ( EthInitStatus == 1 ) /*之前未成功初始化过*/
			{
				/*Reinit PHY*/
				LAN8720_DeInit();
                printf("reinit LAN8720_DeInit\r\n");
			}
			else /*之前已经成功初始化*/
			{
