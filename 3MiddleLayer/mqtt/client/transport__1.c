...
int transport_open(char* addr, int port)
{
    ...
    err_t ret;
    ret = dns_gethostbyname(addr, &host_ip, dns_found, "hello");
    if (ret == ERR_INPROGRESS)
    {//需要通过向dns服务器发送dns请求数据来获取hostname对应的IP地址
        log_d("等待\r\n");
    }
    else
    {//从dns缓存表去除hostname对应的IP地址
        log_d(" %s\r\n", ipaddr_ntoa(&host_ip));
    }
...
//    serv_addr.sin_addr.s_addr = host_ip.addr;
//    memset(&(serv_addr.sin_zero), 0, sizeof(serv_addr.sin_zero));    
//    printf("ip = %d,port = %d\r\n",serv_addr.sin_addr.s_addr,serv_addr.sin_port);  
    
	server = gethostbyname(ipaddr_ntoa(&host_ip));
	if(server == NULL)
