// en_decript.cpp : Defines the entry point for the console application.
//
/*
...
//int Des(uint8 *key,uint8 *input,int inlen,uint8 *output,int type)
//{	
//	unsigned char sInput[8] = {0};
//    unsigned char sTempOut[8] = {0};
//	unsigned char sTempOutData[1024] = {0};
//	unsigned char sTempInput[1024] = {0};	
//	des_context ctx;
//    int inputLen = 0;
//    int i = 0;
//	
//    inputLen = 8 * ( (inlen + 7) / 8 );
//    memcpy(sTempInput,input,inlen);

//    for(i = inlen;i<inputLen;i++)
//        sTempInput[i] = 0x00;
...
//    { 
//        memcpy(sInput,sTempInput+i*8,8);
//		
//		if(type == 0)
//        {
//            des_crypt(ctx.esk,sInput,sTempOut);
//        }
//        else
//        {
//            des_crypt(ctx.dsk,sInput,sTempOut);
//        }
//		
//		memcpy(sTempOutData+i*8,sTempOut,8);
//    }
//    memcpy(output,sTempOutData,inputLen);

//	return inputLen;
//}

void Des3_2(uint8 *key,uint8 *input,int len,uint8 *output,int type)
{
    unsigned char sInput[8] = {0};
    unsigned char sTempOut[8] = {0};
    unsigned char sTempInput[512] = {0};
    unsigned char sTempOutData[512] = {0};
    uint8 key1[8] = {0};
    uint8 key2[8] = {0};
    des3_context ctx3;
    int inputLen = 0;
    int i = 0;
    inputLen = 8 * ( (len + 7) / 8 );
    memcpy(sTempInput,input,len);

    for(i = len;i<inputLen;i++)
        sTempInput[i] = 0x00;
    ...
    {      
		...
		if(type == 0)
        {
            des3_crypt(ctx3.esk,sInput,sTempOut);
        }
        else
        {
            des3_crypt(ctx3.dsk,sInput,sTempOut);
        }
...
//void Des3_3(uint8 *key,uint8 *input,int len,uint8 *output,int type)
//{
//    unsigned char sInput[8] = {0};
//    unsigned char sTempOut[8] = {0};
//    unsigned char sTempInput[1024] = {0};
...
//    des3_set_3keys(&ctx3,key1,key2,key3);
